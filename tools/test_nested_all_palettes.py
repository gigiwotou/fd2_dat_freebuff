...
# 输出目录
output_dir = f"{WORKSPACE}/output/test_nested_all_palettes"
os.makedirs(output_dir, exist_ok=True)

# 测试特定的嵌套DAT tile
nested_indices = [7, 12, 63]
tile_indices = [1, 2, 3]  # 只测试前几个tile
...
for nested_idx in nested_indices:
    ...
    for tile_idx in tile_indices:
        ...
        pal_byte = tile_data[4]  # offset+4处的字节
        
        print(f"  tile {tile_idx}: {w}x{h}, pal_byte=0x{pal_byte:02X}")
        ...
        for pal_idx in PALETTE_INDICES:
            ...
            filename = f"nested_{nested_idx}_tile_{tile_idx}_{w}x{h}_pal{pal_idx}_byte{pal_byte:02X}.png"
            img.save(os.path.join(output_dir, filename))

print(f"\n完成！输出目录: {output_dir}")
print("生成了所有嵌套DAT tile使用不同调色板的图像，可以手动检查哪个调色板是正确的。")