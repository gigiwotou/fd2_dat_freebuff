#!/usr/bin/env python3
"""
测试嵌套DAT tile数据中的offset+5字节作为调色板偏移
"""
import struct
from PIL import Image
import os

WORKSPACE = r"d:\workspace\fd2_dat_freebuff"
dat_path = f"{WORKSPACE}/bin/FDOTHER.DAT"

with open(dat_path, 'rb') as f:
    data = f.read()

def read_dat_resource(file_data, base_offset, index):
    index_offset = base_offset + 4 * index + 6
    if index_offset + 8 > len(file_data):
        return None, 0, 0
    offset0 = struct.unpack_from('<I', file_data, index_offset)[0]
    offset1 = struct.unpack_from('<I', file_data, index_offset + 4)[0]
    size = offset1 - offset0
    if size <= 0 or offset0 >= len(file_data):
        return None, 0, 0
    resource_data = file_data[offset0:offset0 + size]
    return resource_data, offset0, size

def decompress_rle_with_offset(rle_data, width, height, pal_offset):
    """RLE解压缩，应用调色板偏移"""
    output = bytearray(width * height)
    src_pos = 0
    src_len = len(rle_data)
    row_start = 0
    col_pos = 0
    current_row = 0
    
    while current_row < height and src_pos < src_len:
        ctrl = rle_data[src_pos]
        src_pos += 1
        count = (ctrl & 0x3F) + 1
        
        if ctrl & 0x80:
            if ctrl & 0x40:
                col_pos += count
            else:
                for i in range(count):
                    if src_pos < src_len and col_pos < width:
                        pixel = rle_data[src_pos]
                        src_pos += 1
                        # 应用调色板偏移
                        pixel = (pixel + pal_offset) & 0xFF
                        out_pos = row_start + col_pos
                        if out_pos < len(output):
                            output[out_pos] = pixel
                        col_pos += 1
        else:
            if src_pos < src_len:
                fill_value = rle_data[src_pos]
                src_pos += 1
                # 应用调色板偏移
                fill_value = (fill_value + pal_offset) & 0xFF
                for i in range(count):
                    if col_pos < width:
                        out_pos = row_start + col_pos
                        if out_pos < len(output):
                            output[out_pos] = fill_value
                        col_pos += 1
        
        if col_pos >= width:
            current_row += 1
            row_start += width
            col_pos = 0
    
    return bytes(output)

def load_palette(pal_idx):
    """加载指定索引的调色板"""
    pal_data, _, pal_size = read_dat_resource(data, 0, pal_idx)
    if not pal_data or pal_size != 768:
        return None
    
    palette_rgb = []
    for i in range(256):
        r = pal_data[i * 3]
        g = pal_data[i * 3 + 1]
        b = pal_data[i * 3 + 2]
        r = (r << 2) | (r >> 4)
        g = (g << 2) | (g >> 4)
        b = (b << 2) | (b >> 4)
        palette_rgb.append((r, g, b))
    return palette_rgb

# 加载调色板0
palette0 = load_palette(0)

# 测试嵌套DAT 63的tile 3 (24x20)
nested63_data, _, _ = read_dat_resource(data, 0, 63)
tile3_data, _, tile3_size = read_dat_resource(nested63_data, 0, 3)

w = struct.unpack_from('<H', tile3_data, 0)[0]
h = struct.unpack_from('<H', tile3_data, 2)[0]

# offset+5处的值作为调色板偏移
pal_offset = tile3_data[5]
print(f"嵌套DAT 63 tile 3:")
print(f"  尺寸: {w}x{h}")
print(f"  offset+5处的BYTE: {pal_offset} (0x{pal_offset:02X})")

# 用offset+6作为RLE数据开始，应用offset+5作为调色板偏移
rle_data = tile3_data[6:]
decompressed = decompress_rle_with_offset(rle_data, w, h, pal_offset)

output_dir = f"{WORKSPACE}/output/test_pal_offset"
os.makedirs(output_dir, exist_ok=True)

# 保存图像
img = Image.new('RGB', (w, h))
for y in range(h):
    for x in range(w):
        px_idx = y * w + x
        if px_idx < len(decompressed):
            pal_val = decompressed[px_idx]
            if pal_val < 256:
                img.putpixel((x, y), palette0[pal_val])
            else:
                img.putpixel((x, y), (0, 0, 0))

img.save(os.path.join(output_dir, f"nested63_tile3_paloffset{pal_offset}.png"))
print(f"  已保存使用调色板偏移 {pal_offset} 的图像")

# 也测试其他嵌套DAT的tile
print(f"\n测试嵌套DAT 12 tile 3:")
nested12_data, _, _ = read_dat_resource(data, 0, 12)
tile3_data_12, _, tile3_size_12 = read_dat_resource(nested12_data, 0, 3)

w12 = struct.unpack_from('<H', tile3_data_12, 0)[0]
h12 = struct.unpack_from('<H', tile3_data_12, 2)[0]
pal_offset_12 = tile3_data_12[5]

print(f"  尺寸: {w12}x{h12}")
print(f"  offset+5处的BYTE: {pal_offset_12} (0x{pal_offset_12:02X})")

# 用offset+6作为RLE数据开始
rle_data_12 = tile3_data_12[6:]
decompressed_12 = decompress_rle_with_offset(rle_data_12, w12, h12, pal_offset_12)

img12 = Image.new('RGB', (w12, h12))
for y in range(h12):
    for x in range(w12):
        px_idx = y * w12 + x
        if px_idx < len(decompressed_12):
            pal_val = decompressed_12[px_idx]
            if pal_val < 256:
                img12.putpixel((x, y), palette0[pal_val])
            else:
                img12.putpixel((x, y), (0, 0, 0))

img12.save(os.path.join(output_dir, f"nested12_tile3_paloffset{pal_offset_12}.png"))
print(f"  已保存使用调色板偏移 {pal_offset_12} 的图像")
