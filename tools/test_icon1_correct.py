...
def main():
    ...
    for icon_idx in range(min(3, len(icon_offsets)-1)):
        ...
        # 直接使用icon_data解码(没有4字节宽高头)
        print(f"像素数据大小: {len(icon_data)}字节")
        ...
        for _ in range(outer_w * outer_h):
            pixels.append(sub_4ec66_step(icon_data, len(icon_data)))
        ...
        render_image(pixels_with_pal, outer_w, outer_h, palette, 
                    f'output/icon{icon_idx}_no_header.png')
        print(f"已保存: output/icon{icon_idx}_no_header.png")
