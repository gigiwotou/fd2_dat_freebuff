#!/usr/bin/env python3
"""
测试工具：基于IDA MCP分析结果验证地图解析

功能：
1. 使用IDA验证的地形ID计算公式：byte[0] | ((byte[1] & 3) << 8)
2. 使用IDA验证的瓦片偏移表解析：byte 6开始，DWORD条目
3. 使用IDA验证的RLE解压缩：4种模式（SKIP/COPY/FILL/ALTERNATE）
4. 详细的验证输出和对比信息
"""
...
def parse_fdfield_offsets(file_data):
    """
    解析FDFIELD.DAT的资源偏移表
    
    格式：
    - Byte 0-5: 魔数 "LLLLLL"
    - Byte 6+: 资源偏移表，每个条目4字节DWORD（无显式数量）
    - 当偏移值无效时停止
    """
    magic = file_data[:6]
    if magic != b"LLLLLL":
...
def test_map_ida_verified(map_id=0):
    ...
    # 2. 解析FDFIELD资源
    print("[2] 解析FDFIELD资源...")
    fdfield_offsets = parse_fdfield_offsets(fdfield_data)
    ...
    spawn_data = fdfield_data[spawn_start:]  # spawn size not needed for this test
    ...
    fdshap_offsets = parse_fdshap_offsets(fdshap_data)
    ...
    palette_start = fdshap_offsets[palette_res_idx]
    palette_end = fdshap_offsets[palette_res_idx + 1] if palette_res_idx + 1 < fdshap_resource_count else len(fdshap_data)
    ...
    palette_data = fdshap_data[palette_start:palette_end]
    tile_set_data = fdshap_data[tile_set_start:tile_set_end]
    
    print(f"  调色板资源 (#{palette_res_idx}): {len(palette_data)} 字节")
    ...
    output_path = OUTPUT_DIR / f"map_{map_id}_ida_verified.png"
