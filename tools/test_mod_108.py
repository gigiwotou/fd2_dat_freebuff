...
def rle_decompress(src, width, height):
    ...
    for row in range(height):
        row_dst = row*width; count = width
        while count > 0 and p < src_end:
            value = src[p]; p += 1; count_1 = (value&0x3F)+1
            ...
            if bit7 and bit6: row_dst += count_1; count -= count_1 if count>=count_1 else count
                ...
                for i in range(count_1):
                    if count>0 and p<src_end:
                        if row_dst<len(dst): dst[row_dst]=src[p]; row_dst+=1; p+=1; count-=1
            elif not bit7 and bit6:
                if p<src_end:
                    fill=src[p]; p+=1
                    for i in range(count_1):
                        if count>=2:
                            if row_dst+1<len(dst): dst[row_dst+1]=fill; row_dst+=2; count-=2
                        else:
                            if row_dst<len(dst): dst[row_dst]=fill; row_dst+=1; count-=1
            else:
                if p<src_end:
                    fill=src[p]; p+=1
                    for i in range(count_1):
...
tests = [
    ...
    ('% 192', lambda tid: tid % 192),
    ('% 108', lambda tid: tid % 108),
    ('% 128', lambda tid: tid % 128),
    ('% 144', lambda tid: tid % 144),
    ('& 0x7F', lambda tid: tid & 0x7F),
    ('& 0xFF', lambda tid: tid & 0xFF),
]
...
for name, func in tests:
    ...
    for i in range(w*h):
        pos = 4+4*i
        tid = struct.unpack_from('<H',layout,pos)[0]
        tile_idx = func(tid)
    ...
    diff = sum(1 for a,b in zip(orig.getdata(),map_img.getdata()) if a!=b)
    match = (1-diff/(w*h*tile_w*tile_h))*100
    print(f'{name:20s}: rendered={rendered:3d}/576, match={match:.1f}%')
