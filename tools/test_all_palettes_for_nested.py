...
def save_tile_with_palette(nested_idx, tile_idx, w, h, decompressed, palette, palette_name):
    """使用指定调色板保存tile图像"""
    ...
    filename = f"nested_{nested_idx}_tile_{tile_idx}_{w}x{h}_{palette_name}.png"
...
for nested_idx in nested_indices:
    ...
    for tile_idx in range(1, 23):
        ...
        for pal_idx, palette in palettes.items():
            filename = save_tile_with_palette(
                nested_idx, tile_idx, w, h, decompressed, palette, f"pal{pal_idx}"
            )
            if tile_idx <= 3:  # 只打印前3个
