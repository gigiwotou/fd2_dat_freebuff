#!/usr/bin/env python3
"""
测试嵌套DAT tile数据结构中的调色板偏移
假设: [w:2][h:2][pal_offset:1][rle_data...]
offset+4处的BYTE是调色板偏移值
"""
import struct
...
def decompress_rle_with_pal_offset(rle_data, width, height, pal_offset):
    """RLE解压缩，应用调色板偏移"""
...
# 输出目录
output_dir = f"{WORKSPACE}/output/nested_dat_with_pal_offset"
...
for nested_idx in nested_indices:
    ...
    for tile_idx in range(1, 23):
        ...
        decompressed = decompress_rle_with_pal_offset(rle_data, w, h, pal_offset)
        ...
        if tile_idx <= 5:
            print(f"  tile {tile_idx}: {w}x{h}, pal_offset={pal_offset}, 已保存")
