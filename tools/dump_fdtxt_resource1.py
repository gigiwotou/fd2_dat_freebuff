...
# -*- coding: utf-8 -*-
"""
FDTXT.DAT 详细转储工具 - 资源1子项0
解析资源1，子文本0，打印所有int16值及控制码参数
输出：
1. 完整的int16数组（所有值）
2. 标注每个控制码（-1到-20）
3. 统计字符数量和控制码数量
...
def main():
    ...
    file_size = len(data)
    print('=' * 80)
    print('FDTXT.DAT 详细转储 - 资源%d/子文本0' % resource_index)
    print('=' * 80)
    ...
    # 检查目标资源
    if len(offsets) <= resource_index:
        print('错误: 资源集数量不足')
        return
    
    res_start = offsets[resource_index]
    res_end = offsets[resource_index + 1] if resource_index + 1 < len(offsets) else file_size
    
    print('=== 资源%d ===' % resource_index)
    print('偏移: 0x%08X - 0x%08X' % (res_start, res_end))
    print('大小: %d 字节' % (res_end - res_start))
    print()
    
    # 资源数据结构:
    # 前2字节: 子项数量(16-bit signed)
    # 接下来: 子项偏移表(每个是16-bit signed，相对资源起始)
    # 然后: 实际子项数据
    ...
    print('=== 子文本0 ===')
    print('偏移: %d - %d (相对资源%d)' % (sub0_start, sub0_end, resource_index))
    ...
    print('=' * 80)
    print()
    print('总word数量: %d' % index)
    print('普通字符数量: %d' % char_count)
    print('控制码数量: %d' % control_count)
    print()
    print('控制码详情:')
    for code in sorted(control_details.keys()):
        desc = CONTROL_CODES.get(code, '未知控制码')
        print('  %6d (%s): %d次 - %s' % (code, '0x%04X' % (code & 0xFFFF), control_details[code], desc))
    print()
    print('=' * 80)
    print('完整int16数组（逗号分隔）:')
    print('=' * 80)
