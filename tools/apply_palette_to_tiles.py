...
"""
应用正确调色板到提取的tile图像
根据分析，FDOTHER.DAT索引7是768字节的256色调色板
"""
import struct
...
input_dir = f"{WORKSPACE}/output/nested_dat_tiles_v4"
output_dir = f"{WORKSPACE}/output/nested_dat_tiles_v4_colored"
...
# 读取索引7的调色板
palette_data, palette_offset, palette_size = read_dat_resource(data, 0, 7)
print(f"索引7 (调色板):")
print(f"  偏移: {palette_offset}")
print(f"  大小: {palette_size}")

if palette_size != 768:
    print("警告：调色板大小不是768字节")
else:
    print("调色板大小正确 (256色 × 3字节)")
