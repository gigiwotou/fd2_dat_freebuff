#!/usr/bin/env python3
"""
测试嵌套DAT tile的调色板偏移假设
假设tile数据结构: [w:2][h:2][pal_offset:1][rle_data...]
使用offset+5作为RLE数据开始，offset+4作为调色板偏移
"""
import struct
...
output_dir = f"{WORKSPACE}/output/test_pal_offset_v3"
...
tile1_data_12, _, tile1_size_12 = read_dat_resource(nested12_data, 0, 1)

print(f"  前16字节: {' '.join(f'{b:02X}' for b in tile1_data_12[:16])}")

w12 = struct.unpack_from('<H', tile1_data_12, 0)[0]
h12 = struct.unpack_from('<H', tile1_data_12, 2)[0]
pal_offset_12 = tile1_data_12[4]

print(f"  尺寸: {w12}x{h12}")
print(f"  offset+4处的BYTE: {pal_offset_12} (0x{pal_offset_12:02X})")

rle_data_12 = tile1_data_12[5:]
decompressed_12 = decompress_rle_with_palette_offset(rle_data_12, w12, h12, pal_offset_12)
...
img12.save(os.path.join(output_dir, f"nested12_tile1_paloff{pal_offset_12}.png"))
