...
with open(dat_path, 'rb') as f:
    ...
    print(f"\n索引7的文件偏移: 0x{offsets[7]:X}")
    print(f"索引8的文件偏移: 0x{offsets[8]:X}")
    print(f"索引7的大小: {offsets[8] - offsets[7]} 字节")
    
    # 跳转到索引7
    f.seek(offsets[7])
    
    # 读取整个索引7资源
    res_size = offsets[8] - offsets[7]
    ...
    print("\n" + "="*60)
    print("解析尝试2: 简单tile集（无LMI1头）")
    print("="*60)
    
    # 尝试作为WORD数量 + DWORD偏移表
    if len(res_data) >= 2:
        count = struct.unpack('<H', res_data[0:2])[0]
        print(f"  假设WORD[0-1]是tile数量: {count}")
    ...
    print("\n" + "="*60)
    print("解析尝试3: DWORD偏移表（无数量字段）")
    print("="*60)
    
    # 尝试直接作为DWORD偏移表
    print("  前20个DWORD值:")
    for i in range(min(20, res_size // 4)):
        val = struct.unpack('<I', res_data[i*4:i*4+4])[0]
        print(f"  DWORD[{i}]: 0x{val:X} ({val})")
    
    print("\n" + "="*60)
    print("解析尝试4: 直接查找tile头（宽高WORD对）")
    print("="*60)
    
    # 搜索可能的tile头（宽高都是合理值，如<1000）
    print("  搜索可能的tile头（宽高<1000）:")
    found = 0
    for i in range(0, res_size - 4, 2):
        w = struct.unpack('<H', res_data[i:i+2])[0]
        h = struct.unpack('<H', res_data[i+2:i+4])[0]
        if 0 < w < 1000 and 0 < h < 1000:
            print(f"  偏移0x{i:X}: {w}x{h}")
            found += 1
            if found >= 20:
                break
