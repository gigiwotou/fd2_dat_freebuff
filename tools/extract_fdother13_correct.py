...
关键发现：
- sub_2EB9F: v9 = (WORD *)(*(DWORD *)(a5 + 4*a6 + 8) + a5)
- sub_4E98D(v9+9, *v9, v9[1], ...) 
- tile头: [0-1]width, [2-3]height, [4-8]未知, [8-9]未知, [9+]RLE数据

但嵌套DAT中的tile数据没有宽高头，直接是RLE像素数据
这意味着sub_25A96/sub_25B45使用不同的渲染管线，不经过sub_2EB9F
...
def extract_tile_images(dat_path, output_dir):
    ...
    for scene_idx in scene_indices:
        ...
        for tile_idx in range(len(valid_offsets) - 1):
            ...
            # 检查是否是tile格式 [width:2][height:2][? :4][? :2][RLE数据]
            # 从sub_2EB9F: v9+9 是RLE数据起始，*v9是width，v9[1]是height
            # 所以头是10字节
            ...
            for width, height in test_sizes:
                if width * height <= tile_size * 3:  # RLE通常压缩比1:3
                    try:
                        ...
                        print(f"  [尝试] {width}x{height}: {output_path}")
