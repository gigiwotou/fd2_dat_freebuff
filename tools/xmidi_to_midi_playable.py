#!/usr/bin/env python3
"""
Convert XMIDI to MIDI with zero initial delay
All delta times are preserved relative to each other,
but the first event starts at delta=0
"""
...
def extract_xmidi_events(data, remove_initial_delay=True):
    ...
    while pos < end:
        ...
        if remove_initial_delay and first_event:
            delta = 0
            first_event = False
...
def convert_xmidi_to_midi(xmidi_data, remove_initial_delay=True):
    ...
    # Extract events
    events = extract_xmidi_events(xmidi_data[evnt_pos:], remove_initial_delay)
...
def convert_all_tracks(input_dir, output_dir, remove_initial_delay=True):
    ...
    for track_file in sorted(input_dir.glob("track_*.bin")):
        ...
        midi_data = convert_xmidi_to_midi(data, remove_initial_delay)
...
def main():
    ...
    output_dir = Path("output/fdmus_midi_playable")
    
    if not input_dir.exists():
        print("Error: Input directory not found")
        return
    
    print("Converting XMIDI to MIDI with zero initial delay...")
    convert_all_tracks(input_dir, output_dir, remove_initial_delay=True)
    
    print(f"\nPlayable MIDI files saved to: {output_dir}")
