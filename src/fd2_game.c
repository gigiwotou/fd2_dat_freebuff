...
int fd2_game_init(fd2_game_t* game, const char* data_dir) {
    ...
    /* Disable IME text input to prevent interference with game controls.
     * On Windows with Chinese/Japanese IME, pressing keys can trigger
     * IME composition mode which causes SDL to stop generating
...
/* Map tile size in pixels (from FDSHAP.DAT: 24x24 per tile) */
...
typedef struct {
    ...
    int anim_frame;       /* 0-2 animation frame within direction */
...
static void update_map_sprite_animation(map_sprite_t* sprite) {
    ...
    /* Update animation frame counter (cycle 0->1->2->0->1->2...)
     * Per IDA: each direction has 3 frames */
    sprite->anim_timer++;
...
static void state_battle_enter(fd2_game_t* game) {
    ...
    /* Per IDA sub_10010:
     * - from_save=1: loop n6_0 times, data from n8_1 (v5+4771)
     * - from_save=0: loop char_pos_count times, data from FDFIELD.DAT
     */
        ...
        if (data->debug_grid_enabled) {
            ...
            for (int ty = start_tile_y; ty <= end_tile_y; ty++) {
                for (int tx = start_tile_x; tx <= end_tile_x; tx++) {
                    ...
                    /* Draw simple white grid lines (color 63 = white in default palette) */
                    u8 grid_color = 63;
                    
                    /* Horizontal line (top of tile) */
                    if (screen_y >= 0 && screen_y < FD2_SCREEN_H) {
                        ...
                        for (int x = x_start; x < x_end; x++) {
                            game->render.screen[screen_y * FD2_SCREEN_W + x] = grid_color;
                        }
                    }
                    
                    /* Vertical line (left of tile) */
                    if (screen_x >= 0 && screen_x < FD2_SCREEN_W) {
                        ...
                        for (int y = y_start; y < y_end; y++) {
                            game->render.screen[y * FD2_SCREEN_W + screen_x] = grid_color;
                        }
                    }
                }
            }
...
typedef struct {
    ...
    u8 char_icons[64];         /* Icon IDs for each character */
...
static void state_continue_enter(fd2_game_t* game) {
    ...
    if (num_chars > 0 && num_chars <= 64) {
        ...
        for (int i = 0; i < num_chars; i++) {
            ...
            data->char_icons[i] = char_data[7];         /* Icon ID: offset+7 */
