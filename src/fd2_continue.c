...
void state_continue_enter(fd2_game_t* game) {
    ...
    if (num_chars > 0 && num_chars <= 64) {
        ...
        for (int i = 0; i < num_chars; i++) {
            ...
            fprintf(stderr, "  char[%d]: x=%d, y=%d, icon_id=%d, offset+5=0x%02X(bit0=%d,bit7=%d), death_flag=%d, active_mask=0x%02X\n",
                   i, char_data[0], char_data[1], char_data[7], char_data[5], char_data[5] & 1, (char_data[5] >> 7) & 1, char_data[39], char_data[26]);
        }
    } else {
        data->num_fighters = 0;
    }
